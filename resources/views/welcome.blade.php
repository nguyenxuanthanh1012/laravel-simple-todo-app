        ...
        <div class="lg:w-2/4 mx-auto py-8 px-6 bg-white rounded-xl">
            ...
            <div class="mb-6">
                <form action="/" method="post" class="flex flex-col space-y-4">
                    @csrf
                    @method('post')
                    <input type="text" name="title" placeholder="Input something..." class="py-3 px-4 bg-gray-100 rounded-xl">
        ...
        <div class="mt-2">
            ...
            <div class="py-4 flex items-center border-b border-gray-300 px-3">
                <div class="flex-1 pr-8">
                    <h3 class="text-lg font-semibold {{ $todo->isDone? 'line-through' : ''}}">{{$todo->title}}</h3>
                    <p class="text-gray-500">{{$todo->description}}</p>
                </div>
                <div class="flex space-x-3">
                    <form action="{{ route('toggle', ['todo' => $todo]) }}" method="post">
                        @csrf
                        @method('patch')
